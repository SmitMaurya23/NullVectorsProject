...
class A_llmrec_model(nn.Module):
    ...
    def forward(self, data, optimizer=None, batch_iter=None, mode='phase1'):
        if mode == 'phase1':
            ...
            # Directly return the outputs of generate
...
#########################3We modified generate to return a tuple of predictions, relevance scores, and embeddings.#################
import json
...
def generate(self, data):
    ...
    with torch.no_grad():
        ...
        for i in range(len(u)):
            ...
            # For MMR, store recommendations, relevance scores, and embeddings
            recommendations.append(target_item_title)
            relevance_scores[target_item_title] = rank  # Customize based on relevance
            embeddings[target_item_title] = self.get_item_emb([target_item_id]).squeeze(0)
    ...
    with torch.no_grad():
        ...
        outputs[outputs == 0] = 2  # Convert output id 0 to eos_token_id
    ...
    print("Returning from generate:", output_text, recommendations, relevance_scores, embeddings)

    # Inside generate function

    # New: Write predictions to a JSON file
    prediction_data = {
        "text_input": text_input,
        "answers": answer,
        "llm_predictions": output_text,
        "recommendations": recommendations,
        "relevance_scores": relevance_scores,
        "embeddings": {k: v.cpu().tolist() for k, v in embeddings.items()}
    }
    with open('./predictions.json', 'w') as json_file:
        json.dump(prediction_data, json_file, indent=4)

    # Return both the output text and MMR-compatible structures
    return output_text, recommendations, relevance_scores, embeddings
