...
class A_llmrec_model(nn.Module):
    ...
    # Apply Diversity-Penalized Re-Ranking
     diversified_recommendations = []
     lambda_diversity = 0.5  # Penalty term for diversity
     for i in range(len(relevance_scores)):
        ...
        for _ in range(candidate_num):
            if len(selected_items) == 0:
                ...
                max_index = np.argmax(penalized_scores)
            else:
                # Apply diversity penalty
                for j in range(len(candidate_scores)):
                    if j not in selected_items:
                        diversity_penalty = np.mean([cosine_similarity([candidate_embeddings[j]], [candidate_embeddings[k]])[0][0] for k in selected_items])
                        penalized_scores[j] = candidate_scores[j] - lambda_diversity * diversity_penalty
