...
class A_llmrec_model(nn.Module):
    ...
    # Apply Cluster-Based Diversification
     diversified_recommendations = []
     for i in range(len(relevance_scores)):
        candidate_embeddings = candidate_embs[i].cpu().numpy()
        
        # Cluster the candidate items
        num_clusters = min(candidate_num, len(candidate_embeddings))
        kmeans = KMeans(n_clusters=num_clusters, random_state=0).fit(candidate_embeddings)
        ...
        for cluster_id in range(num_clusters):
            ...
            if len(cluster_indices) > 0:
                # Select the item with the highest relevance score within the cluster
